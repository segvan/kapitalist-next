import {printError, round, roundDecimal, roundNumber, runInBatches} from "../helpers"
...
const getBeginDates = async (): Promise<any> => {
  const dates = await db.trades.groupBy(
    {
      by: ["symbol"],
      _max: {timestamp: true},
    }
...
const getMyTrades = async (beginDates: any, symbols: Symbol[]): Promise<Trade[]> => {
...
const saveHistoryAggr = async (): Promise<void> => {
  const tradesAggr = (await db.trades.groupBy(
    {
      by: ["symbol"],
  ...
  await db.tradesAggr.deleteMany({
    where: {
